defmodule ExDoubleEntry.AccountBalanceTest do
  ...
  describe "lock_multi!/2" do
    setup do
      acc_a = :account_balance |> insert(identifier: "savings") |> Account.present()
    ...
    test "failed locks", %{acc_a: acc_a, acc_b: acc_b} do
      [
        Task.async(fn ->
          AccountBalance.lock_multi!([acc_a, acc_b], fn -> :timer.sleep(50) end)
        end),
        Task.async(fn ->
          assert_raise(DBConnection.ConnectionError, fn ->
            AccountBalance.lock_multi!([acc_a, acc_b], fn ->
              Transfer.perform!(
                Money.new(42, :USD),
                from: acc_a, to: acc_b, code: :deposit, metadata: nil
              )
            end)
          end)
        end),
